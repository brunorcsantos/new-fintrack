...
class ApiClient {
  ...
  private async request<T>(
    ...
    if (response.status === 401 && !path.includes("/auth/refresh")) {
      ...
      try {
        ...
        throw { error: "SESSION_EXPIRED", message: "Sessão expirada. Faça login novamente.", statusCode: 401 }
      } finally {
   ...
   */
  private async doRequest(path: string, options: RequestInit): Promise<Response> {
  ...
  async login(email: string, password: string): Promise<void> {
    const tokens = await this.post<TokenPair>("/auth/login", { email, password })
    this.setTokens(tokens)
  }

  async register(name: string, email: string, password: string): Promise<void> {
    const tokens = await this.post<TokenPair>("/auth/register", { name, email, password })
    this.setTokens(tokens)
  }
  ...
  async logout(): Promise<void> {
    if (this.refreshToken) {
      ...
      await this.post("/auth/logout", { refreshToken: this.refreshToken }).catch(() => {})
    }
    this.clearTokens()
  }
